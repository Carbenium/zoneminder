...
void Event::AddFrame(Image *image,
  ...
  if (image) {
    if (save_jpegs & 1) {
      std::string event_file = stringtf(GetStaticConfig().capture_file_format, path.c_str(), frames);
    ...
    if (frame_type == ALARM) {
      ...
      if (alarm_image and (save_jpegs & 2)) {
        std::string event_file = stringtf(GetStaticConfig().analyse_file_format, path.c_str(), frames);
